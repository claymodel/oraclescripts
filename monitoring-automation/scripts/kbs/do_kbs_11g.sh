...
#
#     2012-09-21	Zhang Hua		        V4.0.2	Update
#          1. 为 11g 运行进行了优化：
#             删除了“PROMPT Diag Info Part 4. Local Lock Holders: Session Locks”部分
#
...
sqlplus '/ as sysdba' <<!    >/dev/null
  ...
  select 'max_ctime: ' || nvl( max( ctime ), 0 ) max_ctime
