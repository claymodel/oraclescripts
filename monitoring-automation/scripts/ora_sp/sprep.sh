...
Rem select d.dbid            dbid
Rem      , d.name            db_name
Rem      , i.instance_number inst_num
Rem      , i.instance_name   inst_name
Rem   from v\$database d,
Rem        v\$instance i;
...
set echo off feedback off pagesize 999 linesize 999 newpage 1 recsep off;
...
Rem prompt
Rem prompt
Rem prompt Instances in this Statspack schema
Rem prompt ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Rem select distinct 
Rem        dbid            dbbid
Rem      , instance_number instt_num
Rem      , db_name         dbb_name
Rem      , instance_name   instt_name
Rem      , host_name       host
Rem   from stats\$database_instance;
 
Rem prompt
Rem prompt Using &&dbid for database Id
Rem prompt Using &&inst_num for instance number
...
Rem prompt
Rem prompt
Rem prompt Specify the Begin and End Snapshot Ids
Rem prompt ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Rem prompt Begin Snapshot Id specified: &&begin_snap
Rem prompt
Rem prompt End   Snapshot Id specified: &&end_snap
Rem prompt
...
Rem select parallel       para
Rem      , version        versn
Rem      , host_name      host_name
Rem      , db_name        db_name
Rem      , instance_name  inst_name
Rem      , to_char(snap_time, 'YYYYMMDD HH24:MI:SS')  btime
Rem   from stats\$database_instance di
Rem      , stats\$snapshot          s
Rem  where s.snap_id          = :bid
Rem    and s.dbid             = :dbid
Rem    and s.instance_number  = :inst_num
Rem    and di.dbid            = s.dbid
Rem    and di.instance_number = s.instance_number
Rem    and di.startup_time    = s.startup_time;

Rem select to_char(snap_time, 'YYYYMMDD HH24:MI:SS')  etime
Rem   from stats\$snapshot     s
Rem  where s.snap_id          = :eid
Rem    and s.dbid             = :dbid
Rem    and s.instance_number  = :inst_num;
...
Rem select 'sp_'||:bid||'_'||:eid dflt_name from dual;
set termout on;

Rem prompt
Rem prompt Specify the Report Name
Rem prompt ~~~~~~~~~~~~~~~~~~~~~~~
Rem prompt The default report file name is &dflt_name..  To use this name, 
Rem prompt press <return> to continue, otherwise enter an alternative.

set heading off;
column report_name new_value report_name noprint;
Rem select 'Using the report name ' || nvl('&&report_name','&dflt_name')
Rem     , nvl('&&report_name','&dflt_name') report_name
Rem from sys.dual;
Rem spool &report_name;
set heading on;
Rem prompt
...
Rem prompt  STATSPACK report for
...
Rem select :db_name    db_name
Rem      , :dbid       dbid
Rem      , :inst_name  inst_name
Rem      , :inst_num   inst_num
Rem      , :versn      versn
Rem      , :para       para
Rem      , :host_name  host_name
Rem   from sys.dual;
...
spool $workdir/1.load_profile_$identifier.lst
...
spool $workdir/2.instance_efficiency_percentages_$identifier.lst
...
spool $workdir/3.top_wait_events_$identifier.lst
...
spool $workdir/4.cluster_specific_ratios_$identifier.lst
...
spool $workdir/5.miscellaneous_ges_cluster_statistics_$identifier.lst
...
spool $workdir/6.system_event_$identifier.lst
...
spool $workdir/7.background_process_wait_events_$identifier.lst
...
spool $workdir/8.sql_statements_order_by_buffer_gets_$identifier.lst
...
spool $workdir/9.sql_statements_order_by_physical_reads_$identifier.lst
...
spool $workdir/10.sql_statements_order_by_executions_$identifier.lst
...
spool $workdir/11.sql_statements_order_by_parse_calls_$identifier.lst
...
spool $workdir/12.sql_statements_order_by_shareable_memory_$identifier.lst
...
spool $workdir/13.sql_statements_order_by_version_count_$identifier.lst
...
spool $workdir/14.instance_activity_statistics_$identifier.lst
...
spool $workdir/15.session_wait_events_$identifier.lst
...
spool $workdir/16.session_statistics_$identifier.lst
...
Rem col reads      format 9,999,999,990 heading 'Reads' newline;
...
spool $workdir/17.tablespace_io_stats_$identifier.lst
...
Rem break on tsname skip 1;

spool $workdir/18.file_io_statistics_$identifier.lst
...
spool $workdir/19.buffer_pools_$identifier.lst
...
spool $workdir/20.instance_recovery_stats_$identifier.lst
...
spool $workdir/21.buffer_pool_advisory_$identifier.lst
...
spool $workdir/22.buffer_waits_$identifier.lst
...
spool $workdir/23.pga_cache_hit_percentage_$identifier.lst
...
spool $workdir/24.pga_usage_histogram_$identifier.lst
...
spool $workdir/25.pga_advisory_$identifier.lst
...
spool $workdir/26.enqueue_activity_$identifier.lst
...
spool $workdir/27.rollback_segment_$identifier.lst
...
spool $workdir/28.undo_segment_$identifier.lst
...
spool $workdir/29.latch_activity_$identifier.lst
...
spool $workdir/30.latch_sleep_breakdown_$identifier.lst
...
spool $workdir/31.latch_miss_sources_$identifier.lst
...
spool $workdir/32.parent_latch_$identifier.lst
...
spool $workdir/33.latch_children_$identifier.lst
...
spool $workdir/34.segment_statistics_$identifier.lst
...
spool $workdir/35.dictionary_cache_$identifier.lst
...
spool $workdir/36.library_cache_$identifier.lst
...
spool $workdir/37.shared_pool_advisory_$identifier.lst
...
spool $workdir/38.sga_$identifier.lst
...
spool $workdir/39.resource_limit_$identifier.lst
...
spool $workdir/40.initialization_parameters_$identifier.lst
